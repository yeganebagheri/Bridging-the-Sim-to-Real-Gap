...
class Agent(object): #The Agent class is designed to interact with the environment using a given policy, collect experiences, and update the policy based on these experiences.
    ...
    def update_policy(self):
        ...
        baseline = 20 #change to 0 for REINFORCE without a baseline
        discount_reward = discount_rewards(rewards, self.gamma)
        discount_reward = discount_reward - baseline
        loss = -torch.mul(discount_reward, action_log_probs).mean()
        loss.backward()
        self.optimizer.step()
